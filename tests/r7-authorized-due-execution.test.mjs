...
test("authorized runtime defaults launch jitter to zero for deterministic acceptance",async()=>{
  const store=setup(),pub=publisher(store),now=clock();
  try{
    const worker=new AuthorizedRuntimeDueExecutionAdapter(
      store,pub,gate(),
      ()=>({mode:"canary",allowFinalPublish:true,allowedAccountIds:new Set(["acct"]),releaseSha:"release-1"}),
      {releaseSha:"release-1",ownerId:"worker",clock:now,maxPerCycle:1}
    );
    const report=await worker.runDue(target);
    assert.equal(report.claimed,1);
    assert.equal(report.verified,1);
    assert.equal(pub.clicks,1);
  }finally{store.close();}
});
