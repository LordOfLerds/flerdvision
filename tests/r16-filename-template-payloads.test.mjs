...
async function renderedPayloads(specOverride) {
  ...
  try {
    const spec = specOverride ?? exampleSpec();
...
test("the shipped example posts no marker, and the tail appears only where a spec asks for it", async () => {
  const spec = exampleSpec();
  const payloads = await renderedPayloads();
  for (const channel of spec.channels) {
    assert.equal(channel.formats[0].verificationMarker, false, "production posts carry no visible marker");
    const text = payloads[channel.platform].caption ?? payloads[channel.platform].title;
    assert.doesNotMatch(text, /\[FV:/);
  }
  // The marker path itself is unchanged: a format that still asks for it still gets the tail.
  const markerSpec = parseWorkspaceSpec({
    ...JSON.parse(readFileSync(new URL("../config/flerdvision.example.json", import.meta.url), "utf8")),
    channels: [{
      key: "ig-marker", name: "Marker", platform: "instagram", handle: "flerdvision",
      formats: [{ type: "reel", times: ["12:00"], sourceMatch: ["reels"], hashtags: [], verificationMarker: true, requirement: "REQUIRED", settings: {} }]
    }]
  });
  const marked = await renderedPayloads(markerSpec);
  assert.match(marked.instagram.caption, new RegExp(`\\[FV:${CONTENT_ID}\\]$`), "verificationMarker: true still appends the tail to the default template");
});
