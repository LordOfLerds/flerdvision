...
const registry=new JsonWorkspaceRegistry(resolve(runtimeRoot,"registry","workspaces.json"));const server=new SelfServiceHttpServer(registry,{runtimeRoot,repoRoot,password,username:arg("--username")??"flerdvision",host:arg("--host")??"127.0.0.1",port:Number(arg("--port")??"8788"),chromiumExecutablePath:arg("--chromium")??process.env.CHROMIUM_EXECUTABLE_PATH??"/usr/bin/chromium",testRunner:new FixedCommandTestRunner()});
