...
function parse(argv:readonly string[]):Args{
  const port=Number(value(argv,"--port")??process.env.FLERDVISION_CONTROL_PORT??8790);if(!Number.isInteger(port)||port<1||port>65535)throw new Error("--port must be a valid TCP port");
  const releaseSha=(value(argv,"--release-sha")??process.env.FLERDVISION_RELEASE_SHA)?.trim();
  return{runtimeRoot:resolve(value(argv,"--runtime-root")??process.env.FLERDVISION_RUNTIME_ROOT??"runtime"),workspaceId:required("--workspace-id",value(argv,"--workspace-id")??process.env.FLERDVISION_WORKSPACE_ID),password:required("--password / FLERDVISION_CONTROL_PASSWORD",value(argv,"--password")??process.env.FLERDVISION_CONTROL_PASSWORD),username:value(argv,"--username")??process.env.FLERDVISION_CONTROL_USERNAME??"flerdvision",host:value(argv,"--host")??process.env.FLERDVISION_CONTROL_HOST??"127.0.0.1",port,...(releaseSha?{releaseSha}:{})};
}
...
async function main():Promise<void>{
  ...
  const stop=()=>{void server.stop().finally(async()=>{routeTests?.close();incidentOperator.close();await channelOperator.close();effectiveChanges.close();sourceRuntime.close();sourceActivation.close();runtime.close();});};process.on("SIGINT",stop);process.on("SIGTERM",stop);
}
