...
interface Args {runtimeRoot:string;workspaceId:string;password:string;username:string;host:string;port:number;calibrationPort:number;releaseSha?:string;}
...
function parse(argv:readonly string[]):Args{
  const port=tcpPort(value(argv,"--port")??process.env.FLERDVISION_CONTROL_PORT??"8790","--port"),calibrationPort=tcpPort(value(argv,"--calibration-port")??process.env.FLERDVISION_CALIBRATION_PORT??"8791","--calibration-port");
  if(port===calibrationPort)throw new Error("Control Center and Calibration UI ports must differ");
  const releaseSha=(value(argv,"--release-sha")??process.env.FLERDVISION_RELEASE_SHA)?.trim();
  return{runtimeRoot:resolve(value(argv,"--runtime-root")??process.env.FLERDVISION_RUNTIME_ROOT??"runtime"),workspaceId:required("--workspace-id",value(argv,"--workspace-id")??process.env.FLERDVISION_WORKSPACE_ID),password:required("--password / FLERDVISION_CONTROL_PASSWORD",value(argv,"--password")??process.env.FLERDVISION_CONTROL_PASSWORD),username:value(argv,"--username")??process.env.FLERDVISION_CONTROL_USERNAME??"flerdvision",host:value(argv,"--host")??process.env.FLERDVISION_CONTROL_HOST??"127.0.0.1",port,calibrationPort,...(releaseSha?{releaseSha}:{})};
}
