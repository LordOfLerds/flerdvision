...
import type { VerificationEvidenceCollectorPort } from "../../domain/verification-ports.js";
...
export class WorkspaceProfileVerificationCollector implements VerificationEvidenceCollectorPort {
  ...
  async collect(intent:PublicationIntent,attempt:PublishAttempt):Promise<readonly VerificationEvidence[]>{
    ...
    const collector=new DeclarativeProfileVerificationCollector(this.store,this.browser,this.locks,new ConfiguredDomSessionProbe(probeEntry.config),this.artifacts,profileEntry.spec,{ownerId:this.ownerId,headless:this.headless,now:this.now});
