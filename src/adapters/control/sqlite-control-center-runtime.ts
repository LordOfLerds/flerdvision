...
function missingPlan(businessDate:string):DailyPlan{return{planId:`daily-plan:missing:${businessDate}`,businessDate,generatedAt:new Date(0).toISOString(),deliveries:[],gaps:[],backlog:[]};}
...
export class SqliteControlCenterRuntimeAdapter implements ControlCenterRuntimePort {
  ...
  private readonly polling:SqliteSourcePollingStateStore;
  ...
  constructor(databasePath:string,private readonly config:DistributionConfigurationStorePort,workspaceId:string="control-center"){
    ...
    this.polling=new SqliteSourcePollingStateStore(databasePath);
  ...
  async snapshot(businessDate:string):Promise<ControlCenterRuntimeSnapshot>{
    ...
    const accounts=this.control.listSocialAccounts().map((record)=>record.account),identities=this.control.listBrowserIdentities();
    const accountHealth:ChannelReadiness[]=accounts.map((account)=>{
      const identity=identities.find((record)=>record.identity.accountId===account.accountId)?.identity,health=identity?this.control.latestSessionHealth(identity.identityId):null;
      return{accountId:account.accountId,sessionHealth:health?.state??"UNKNOWN",identityVerified:Boolean(health?.state==="HEALTHY")};
    });
    const pairs=new Map<string,{accountId:string;postingProfileId:string}>();for(const route of stored.config.routes)pairs.set(`${route.accountId}|${route.postingProfileId}`,{accountId:route.accountId,postingProfileId:route.postingProfileId});
    const surfaceReadiness:SurfaceReadiness[]=[];
    for(const pair of pairs.values()){
      const version=this.surfaces.latestContract(pair.accountId,pair.postingProfileId);if(!version){surfaceReadiness.push({...pair,surfaceContract:"UNVERIFIED"});continue;}
