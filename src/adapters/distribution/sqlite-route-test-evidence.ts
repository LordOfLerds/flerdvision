...
interface EvidenceRow {evidence_id:string;route_id:string;test_key:string;status:string;checked_at:string;release_sha:string;surface_contract_id:string|null;summary:string;artifact_refs_json:string;payload_hash?:string;}
function recordFromRow(row:EvidenceRow):RouteTestEvidenceRecord{return{evidenceId:row.evidence_id,routeId:row.route_id,testKey:row.test_key as RouteTestEvidenceRecord["testKey"],status:row.status as RouteTestEvidenceRecord["status"],checkedAt:row.checked_at,releaseSha:row.release_sha,...(row.surface_contract_id?{surfaceContractId:row.surface_contract_id}:{}),summary:row.summary,artifactRefs:JSON.parse(row.artifact_refs_json) as string[]};}
...
export class SqliteRouteTestEvidenceStore implements RouteTestEvidenceStorePort {
  ...
  record(record:RouteTestEvidenceRecord):RouteTestEvidenceRecord{
    ...
    const existing=this.db.prepare("SELECT payload_hash,evidence_id,route_id,test_key,status,checked_at,release_sha,surface_contract_id,summary,artifact_refs_json FROM route_test_evidence WHERE evidence_id=?")
    ...
    this.db.prepare("INSERT INTO route_test_evidence(evidence_id,route_id,test_key,status,checked_at,release_sha,surface_contract_id,summary,artifact_refs_json,payload_hash) VALUES (?,?,?,?,?,?,?,?,?,?)")
      .run(normalized.evidenceId,normalized.routeId,normalized.testKey,normalized.status,normalized.checkedAt,normalized.releaseSha,normalized.surfaceContractId??null,normalized.summary,JSON.stringify(normalized.artifactRefs),digest);
    return normalized;
  }

  list(routeId:string):readonly RouteTestEvidenceRecord[]{
    const rows=this.db.prepare("SELECT evidence_id,route_id,test_key,status,checked_at,release_sha,surface_contract_id,summary,artifact_refs_json FROM route_test_evidence WHERE route_id=? ORDER BY checked_at,evidence_id").all(routeId) as EvidenceRow[];
