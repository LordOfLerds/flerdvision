...
export class CalibratedWorkspaceRouteTestRunner implements CapabilityAwareRouteTestExecutionAdapterPort {
  ...
  private route(routeId:string){const route=this.config.load().config.routes.find(item=>item.routeId===routeId);if(!route)throw new Error(`Unknown route: ${routeId}`);return route;}
