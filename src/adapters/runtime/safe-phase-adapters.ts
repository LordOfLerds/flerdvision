...
export class W6RuntimeOperationsAdapter implements RuntimeOperationsPort {
  ...
  async projectAndNotify(now:string){
    ...
    if(this.channelKeys.length===0){
      ...
      return{incidentsCreated:projection.created,notificationsEnqueued:0};
    }
    ...
    return{incidentsCreated:report.projection.created,notificationsEnqueued:Math.max(0,after-before)};
  }
}
