import { findLatestRecording } from "../browser/prepare-artifacts.js";
...
export class WorkspacePrivateE2ECommands implements PrivateE2ECommandPort {
  ...
  constructor(private readonly options:WorkspacePrivateE2EOptions){
    ...
    const env=options.env??process.env,chromiumExecutablePath=options.chromiumExecutablePath??env.CHROMIUM_EXECUTABLE_PATH??resolveChromiumExecutablePath();this.env=env;
  ...
  async verify(runId:string,now:string):Promise<string>{const {run,record}=this.intentForRun(runId),attempt=this.store.listPublishAttempts(record.intent.intentId).at(-1);if(!attempt)throw new Error("Private E2E has no publish attempt to verify");const result=await this.publisher.reconciliation.reconcile(record.intent.intentId,attempt.attemptId,this.actor()),passed=result.decision.outcome==="VERIFIED",gate=this.recordGate(run,"VERIFICATION",passed?"PASS":"FAIL",now,`verification ${result.decision.outcome}`,[],{decisionId:result.decision.decisionId,collectorErrors:result.collectorErrors});
    // The operator hears about every post-boundary outcome on the configured channels: VERIFIED
    // with permalink + the verification screenshot, UNCERTAIN as an error that names the freeze.
    const evidence=this.store.listVerificationEvidence(record.intent.intentId,attempt.attemptId).filter(item=>item.positive).at(-1);
    const adapters=[...(telegramAdapterFromEnv(this.env)?[telegramAdapterFromEnv(this.env)!]:[])];
    await notifyPublicationOutcome(this.store,adapters,{intent:record.intent,runId,outcome:passed?"VERIFIED":"UNCERTAIN",...(this.env.TZ?{timeZone:this.env.TZ}:{}),...(result.publication?.permalink?{permalink:result.publication.permalink}:evidence?.locator?{permalink:evidence.locator}:{}),...(evidence?.artifactRef?{screenshotPath:evidence.artifactRef}:{}),...((()=>{try{const v=findLatestRecording(resolve(this.layout.evidenceDir,"publisher"),record.intent.intentId);return v?{videoPath:v}:{};}catch{return{};}})())},new Date(now).toISOString(),this.actor());
    return gate.summary;}
