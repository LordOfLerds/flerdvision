...
export interface RetainedSurfacePublishSession {
  ...
  finalActionLocators:readonly UiLocator[];
  close():Promise<void>;
}
...
export interface SurfacePublishPreparationOptions {
  releaseSha:string;
  ownerId:string;
  headless?:boolean;
  now?:()=>string;
...
/**
 * Canonical reversible preparation path for both Private E2E and future runtime due execution.
 * It may reach the final-action boundary but has no code path that invokes it.
 */
export class SurfacePublishPreparationService {
  private readonly now:()=>string;
  constructor(
    private readonly store:SurfacePublishStore,
    private readonly contextResolver:DistributionPostingContextResolverPort,
    ...
    private readonly sessionProbe:SessionProbeResolver,
    private readonly payloads:PublicationPayloadResolverPort,
    private readonly media:MediaMaterializerPort,
    private readonly artifacts:PrepareArtifactSinkPort,
    private readonly registry:RetainedSurfacePublishSessionRegistry,
    private readonly options:SurfacePublishPreparationOptions
  ){
    if(!options.releaseSha.trim())throw new SurfacePublishSessionError("Surface preparation requires releaseSha");
    this.now=options.now??(()=>new Date().toISOString());
  }

  async prepare(intentIdValue:string,at:string,actor:Actor):Promise<PublishAttempt>{
    const startedAt=new Date(at).toISOString(),record=this.store.getIntent(intentIdValue);
    if(!record)throw new SurfacePublishSessionError(`Unknown publication intent: ${intentIdValue}`);
    if(record.state!=="SCHEDULED")throw new SurfacePublishSessionError(`Surface preparation requires SCHEDULED intent, got ${record.state}`);
    const context=this.contextResolver.resolve(record.intent),surface=this.surfaces.latestContract(record.intent.accountId,context.postingProfile.postingProfileId);
    if(!surface||surface.contract.status!=="CALIBRATED")throw new SurfacePublishSessionError(`Intent ${intentIdValue} requires a CALIBRATED surface contract`);
    const identities=this.store.listBrowserIdentities().map(item=>item.identity).filter(item=>item.accountId===record.intent.accountId&&item.enabled);
    if(identities.length!==1)throw new SurfacePublishSessionError(`Account ${record.intent.accountId} requires exactly one enabled browser identity; found ${identities.length}`);
    ...
    const plan=buildPlatformExecutionPlan(context,surface.contract),probe=this.sessionProbe(record.intent),payload=await this.payloads.resolve(record.intent),content=this.store.getContentItem(record.intent.contentId)?.item;
    if(!content)throw new SurfacePublishSessionError(`Content ${record.intent.contentId} not found`);

    this.store.transitionIntent(intentIdValue,"PREPARING",startedAt,actor,"surface_prepare_started");
    ...
    try{
      lock=this.profileLocks.acquire(identity,this.options.ownerId,startedAt);
      page=await this.browser.launch(identity,{headless:this.options.headless??true,initialUrl:"about:blank"});
      await new BrowserSessionHealthService(this.store,probe).check(identity.identityId,page,this.now(),actor);
      new AccountIdentityGuard(this.store).assertReady(identity.identityId);
      materialized=await this.media.materialize(content);
      const execution=await new SafePlatformExecutionRunner(page,this.artifacts,this.now).execute(plan,identity,{mediaPath:materialized.localPath,...(payload.caption!==undefined?{caption:[payload.caption,...(payload.hashtags??[]).map(tag=>`#${tag}`)].filter(Boolean).join(payload.hashtags?.length?" ":"")}:{}),...(payload.title!==undefined?{title:payload.title}:{})});
      if(!execution.reachedFinalActionBoundary||execution.finalActionInvoked)throw new SurfacePublishSessionError("Surface preparation did not stop safely at final boundary");
      if(execution.environmentFingerprint!==surface.contract.environment.fingerprint)throw new SurfacePublishSessionError("Surface environment changed during preparation");
      ...
      const storedAttempt=this.store.recordPreparedAttempt(attempt,actor),retainedSession:RetainedSurfacePublishSession={attempt:storedAttempt,intent:record.intent,identityId:identity.identityId,surfaceContractId:surface.contract.contractId,environmentFingerprint:surface.contract.environment.fingerprint,session:page,finalActionLocators:finalLocators(plan),close};
      this.registry.add(retainedSession);retained=true;return storedAttempt;
    }catch(error){
      if(!retained)await close();
      const current=this.store.getIntent(intentIdValue);if(current?.state==="PREPARING")this.store.transitionIntent(intentIdValue,"BLOCKED",new Date(this.now()).toISOString(),actor,`surface_prepare_failed:${error instanceof Error?error.message:String(error)}`);
      throw error;
    }
  }
}

/** Final click only; authorization + irreversible-boundary persistence remain outside this class. */
export class RetainedSurfaceFinalActionInvoker implements FinalActionInvokerPort {
  ...
  async invoke(intent:PublicationIntent,attempt:PublishAttempt):Promise<{invokedAt:string;finishedAt:string;evidence:readonly VerificationEvidence[]}>{
    ...
    if(retained.intent.intentId!==intent.intentId||attempt.intentId!==intent.intentId)throw new SurfacePublishSessionError("Retained surface session intent mismatch");
    if(retained.identityId!==attempt.browserIdentityId)throw new SurfacePublishSessionError("Retained surface session browser identity mismatch");
    if(attempt.releaseSha!==retained.attempt.releaseSha)throw new SurfacePublishSessionError("Retained surface session release mismatch");
    const environment=await this.recorder.environment(retained.session);if(environment.fingerprint!==retained.environmentFingerprint)throw new SurfacePublishSessionError("Surface environment changed before final action");
    const invokedAt=new Date(this.now()).toISOString();
    try{
      const descriptor=await new BrowserDomUiDriver(retained.session).clickIrreversible(retained.finalActionLocators,10_000),finishedAt=new Date(this.now()).toISOString();
      return{invokedAt,finishedAt,evidence:[{evidenceId:evidenceId(intent.intentId,attempt.attemptId,invokedAt),intentId:intent.intentId,attemptId:attempt.attemptId,kind:"ui_receipt",observedAt:finishedAt,positive:true,locator:descriptor,note:`Final UI action invoked from CALIBRATED SurfaceContract ${retained.surfaceContractId}; receipt alone is not verification of publication.`}]};
