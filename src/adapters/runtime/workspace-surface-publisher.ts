...
export interface WorkspaceSurfacePublisherOptions {
  runtimeRoot:string;
  workspaceId:string;
  releaseSha:string;
...
export class WorkspaceSurfacePublisher {
  ...
  constructor(options:WorkspaceSurfacePublisherOptions){
    ...
    const env=options.env??process.env,layout=workspaceRuntimeLayout(resolve(options.runtimeRoot),options.workspaceId),sessionProbePath=resolve(layout.configDir,"session-probes.json"),payloadPath=resolve(layout.configDir,"copy-payloads.json");
    if(!existsSync(sessionProbePath))throw new Error(`Workspace session-probes.json is missing: ${sessionProbePath}`);
    if(!existsSync(payloadPath))throw new Error(`Workspace copy-payloads.json is missing: ${payloadPath}`);
    ...
    this.provenance=new SqliteDistributionProvenanceStore(layout.databasePath);
    this.surfaces=new SqlitePlatformSurfaceStore(layout.databasePath);
    const context=new DistributionPostingContextResolver(this.provenance,config),resolver=new BrowserProfileDirectoryResolver(layout.profilesDir),locks=new DurableBrowserProfileLockAdapter(this.control,new FileBrowserProfileLockAdapter(resolver));
    const chromium=new ChromiumCdpRuntimeAdapter({profilesRoot:layout.profilesDir,executablePath:options.chromiumExecutablePath??env.CHROMIUM_EXECUTABLE_PATH??resolveChromiumExecutablePath()});
    ...
    const payloads=new WorkspacePublicationPayloadResolver(payloadPath,this.control),drive=workspaceDriveAccessTokenProvider({configDir:layout.configDir,env}),media=new WorkspaceMediaMaterializer(config,drive,resolve(layout.mediaCacheDir,"publisher")),artifacts=new LocalPrepareArtifactSink(resolve(layout.evidenceDir,"publisher"));
    const now=options.now??(()=>new Date().toISOString());
    this.prepare=new SurfacePublishPreparationService(this.control,context,this.surfaces,chromium,locks,probe,payloads,media,artifacts,this.registry,{releaseSha:options.releaseSha,ownerId:options.ownerId??`${options.workspaceId}:surface-publisher`,headless:options.headless??true,now});
