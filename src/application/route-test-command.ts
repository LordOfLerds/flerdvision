...
export class PersistingRouteTestCommandService implements RouteTestCommandPort {
  ...
  async run(routeId:string,testKey:ExecutableRouteTestKey,now:string):Promise<RouteTestCommandResult>{
    const capability=this.capabilities(routeId).find(item=>item.testKey===testKey);if(!capability?.executable)throw new Error(capability?.reason??`Route test ${testKey} is unavailable`);
    const route=this.config.load().config.routes.find(item=>item.routeId===routeId);if(!route)throw new Error(`Route ${routeId} does not exist`);
    const surface=this.surfaces.latestContract(route.accountId,route.postingProfileId),scope={releaseSha:this.releaseSha,...(surface?{surfaceRecordedAt:surface.recordedAt,surfaceContractId:surface.contract.contractId}:{})};
    const evidence=await this.execution.run(routeId,testKey,this.releaseSha,now,scope),latestRoute=this.config.load().config.routes.find(item=>item.routeId===routeId);if(!latestRoute)throw new Error(`Route ${routeId} disappeared after route test`);
    const latestSurface=this.surfaces.latestContract(latestRoute.accountId,latestRoute.postingProfileId),readiness=this.execution.readiness(routeId,{releaseSha:this.releaseSha,...(latestSurface?{surfaceRecordedAt:latestSurface.recordedAt,surfaceContractId:latestSurface.contract.contractId}:{})});
    this.runtime.putRouteTestReadiness(readiness,new Date(now).toISOString());return{evidence,readiness};
  }
}
