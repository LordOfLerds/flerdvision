...
export interface RuntimeDueExecutionReport {
  claimed: number;
  prepared: number;
  verified: number;
  uncertain: number;
  blocked: number;
  frozen?: number;
  /** Intents auto-waived this cycle because their outage catch-up window elapsed unclaimed. */
  waived: number;
  waivedIntentIds: readonly string[];
}
