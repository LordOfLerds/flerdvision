...
const channels = [
  { key: "reels", name: "LordOfLerds Reels", platform: "instagram", accountId: "account:instagram:instagram-lordoflerds" },
  { key: "clips", name: "LordOfLerds Clips", platform: "tiktok", accountId: "account:tiktok:tiktok-lordoflerds" }
];
...
const incident = {
  incidentId: "incident:inc1", fingerprint: "f", kind: "SOURCE_BLOCKED", severity: "ERROR",
  title: "Content source blocked", summary: "asset:xyz media probe failed",
  scope: { intentId: "intent:i3", accountId: channels[0].accountId }, evidenceRefs: ["/workspaces/ws/evidence/probe.png"],
...
export async function operatorMessageSamples() {
  ...
  const stores = { control, state: { listAssets: () => assets }, pauses: operatorState };
  ...
  const waveWithFailure = publicationWaveMessage([
    ...
    outcome({
      ...
      nextStep: "Im Terminal prüfen, bevor irgendetwas wiederholt wird."
    })
  ], NOW);
  ...
  const enqueued = [];
  new IncidentNotificationService({ enqueueNotification: (message) => { enqueued.push(message); return []; } }, ["telegram"])
