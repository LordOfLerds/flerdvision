...
- W8A self-service productization: **implemented locally; real host promotion starts on Luca Mac**
- Automated tests: **114 passed / 0 failed** at current W8A checkpoint
- Host promotion order: **Luca Mac -> Fabian Mac -> VPS staging -> VPS production-ready**
- Real platform selector/final-action calibration and private publish E2E: **Luca-Mac acceptance next**
...
17. `docs/19-W8-MULTIPLATFORM-E2E-CAMPAIGN.md`
18. `docs/20-W8A-SELF-SERVICE-PRODUCTIZATION.md`
19. `docs/adr/0006-private-e2e-retained-session-final-action.md`

## Current code

The repository now contains the durable control plane, replaceable ingress/disposition adapters, persistent browser identities, the PREPARE_ONLY platform UI kernel, W5 publish-attempt/evidence/reconciliation semantics, W6 incidents/notifications/human recovery/kill switches, the W7 sanitized AI-assisted repair engineering loop, a W8 private-E2E safety harness, and the W8A workspace/self-service host-promotion layer. W4 remains physically non-publishing. W8 can retain the exact prepared browser session and exposes a one-shot test-only final-action path only after W8 gates and W5 durable-boundary persistence. Real platform selectors, login, private publish verification, cleanup and failure injection remain pending on the intended private test host; customer publishing remains blocked.
...
## Self-service / host qualification
...
Then start the private setup UI on `127.0.0.1:8788`. Run the same release independently on Fabian's Mac. A release cannot qualify `FABIAN_MAC` before the same SHA passed `LUCA_MAC`, and it cannot qualify `VPS_STAGING` before `FABIAN_MAC`. See `docs/20-W8A-SELF-SERVICE-PRODUCTIZATION.md`.
